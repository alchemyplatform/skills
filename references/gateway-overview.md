...
See [gateway-reference](gateway-reference.md) for all endpoints, supported chains, and available methods.
...
4. **Send a request** — Call any gateway route with the `Authorization: SIWE <token>` header. For quick queries without an npm project, see [gateway-curl-workflow](gateway-curl-workflow.md) for a lightweight curl-based alternative.
